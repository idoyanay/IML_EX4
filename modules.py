...
# ----------------------------------------------------------
# TODO:
#       1. check that the weights in the 'include_intercept_option in RegularizedModule' are set correctly
# -----------------------------------------------------------
class L2(BaseModule):
    """
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        """
        return np.array([np.linalg.norm(self.weights, ord=2) ** 2])
...
class RegularizedModule(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        output: ndarray of shape (1,)
            Value of function at point self.weights
        """
        fidelity_value = self.fidelity_module_.compute_output(**kwargs)
        regularization_value = self.regularization_module_.compute_output(**kwargs)
        return fidelity_value + self.lam_ * regularization_value
